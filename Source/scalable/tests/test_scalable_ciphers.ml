...
let generate_keys_rsa_tests () =
    let cases = [((from_int 9967, from_int 9973), true)]
...
let encrypt_rsa_tests () =
    let cases =
        [((from_int 281237, (from_int 99400891, from_int 36199003)), from_int 70133953)]
...
let decrypt_rsa_tests () =
    let cases =
        [((from_int 70133953, (from_int 99400891, from_int 30869683)), from_int 281237)]
    and do_check ((m, (n, e)), expected) =
        check
            (list int)
...
let el_gamal_tests () =
    let cases = [(from_int 100000007, from_int 42)]
    and do_check (p, msg) =
